...
void LcdDisplay::begin(uint8_t pinRS, uint8_t pinEN, uint8_t pinD0, uint8_t pinD1, uint8_t pinD2, uint8_t pinD3, uint8_t pinD4, uint8_t pinD5, uint8_t pinD6, uint8_t pinD7)
{
...
void LcdDisplay::print(uint8_t posX, uint8_t posY, String text, bool leftAligned)
{
    ...
    if (!leftAligned)
        posX = textLenght > posX ? 0 : posX - textLenght + 1;

    if ((posX + textLenght) > cols)
